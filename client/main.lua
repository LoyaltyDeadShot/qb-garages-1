...
function JobMenuGarage(garageName)
    ...
    for veh, label in pairs(vehicles) do
        JobVehMenu.options[#JobVehMenu.options+1] = {
            title = label,
            description = "",
    ...
    lib.registerContext(JobVehMenu)
    lib.showContext('JobVehList')
end
...
RegisterNetEvent("qb-garages:client:GarageMenu", function(data)
    ...
    QBCore.Functions.TriggerCallback("qb-garage:server:GetGarageVehicles", function(result)
        if result == nil then
            ...
            for k, v in pairs(result) do
                ...
                if type == "depot" then
                    MenuGarageOptions.options[#MenuGarageOptions.options+1] = {
                        ...
                        description = Lang:t('menu.text.depot', {value = v.plate, value2 = currentFuel, value3 = enginePercent, value4 = bodyPercent, value5 = v.state}),
            ...
            lib.registerContext(MenuGarageOptions)
            lib.showContext('GarageOptionsMenu')
        end
